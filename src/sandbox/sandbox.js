customElements.define('code-sandbox', class extends HTMLElement {
	...
	constructor() {
		...
		this.html = !!html ? html.value : false;
		this.css = !!css ? css.value : false;
		this.js = !!js ? js.value : false;
		...
		this.innerHTML =
			...
			<div class="sandbox">
				...
				<div class="sandbox-content">
					...
					<div class="${this.result === 'iframe' ? 'sandbox-result' : 'sandbox-console-result'}">
						<iframe class="sandbox-iframe" id="sandbox-iframe-${this.uuid}" sandbox="allow-forms allow-modals allow-pointer-lock allow-popups allow-same-origin allow-scripts allow-top-navigation-by-user-activation allow-downloads" frameborder="0" ${this.result === 'console' ? 'hidden' : ''}></iframe>
						${this.result === 'console' ? `<div class="sandbox-label">Console</div>${logger}` : ''}
					</div>
				</div>
				${this.console && this.result !== 'console' ? `<details class="sandbox-console" ${this.getAttribute('console') === 'open' ? 'open' : ''}><summary>Console</summary>${logger}</details>` : ''}
			</div>
		...
		this.loggerElem = this.querySelector(`#sandbox-console-log-${this.uuid}`);
	...
	createEditor(type, elem) {
		...
		return `<details ${(elem && elem.hasAttribute("open")) || isSingleContentType ? "open" : ""}>
				...
				<div class="sandbox-editor">
					<pre class="sandbox-mirror"><code id="sandbox-${type}-mirror-${this.uuid}" class="lang-${type}"></code></pre>
					<textarea spellcheck="false" autocorrect="off" autocapitalize="off" translate="no" class="sandbox-text" id="sandbox-${type}-${this.uuid}"></textarea>
				</div>
			</details>`;
	}
	...
	parseLog(item, depth = 0) {
		...
		if (typeof item !== 'string' && Symbol.iterator in Object(item)) {
			return `[\n${Array.from(item).map(function (val) {
				return `${indentProps}${instance.parseLog(val, depth + 1)}`;
			}).join(',\n')}\n${indent}]`;
		}
		return item && item.nodeType === 1 ? `${item.tagName.toLowerCase()}${item.id ? `#${item.id}` : ''}${item.className ? `.${item.className}` : ''}` : item;
	}
